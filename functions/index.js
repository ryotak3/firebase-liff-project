  ...
  (request, response) => {
    ...
    function showMenu(agent) {
      return db
        ...
        .get()
        .then((snapshot) => {
          if (snapshot.empty) return agent.add(`ああ、ピザはもう出ました。:'(`);
          let menu;
          if (agent.requestSource !== agent.LINE) {
            menu = `お店の美味しいピザのページは：\n${snapshot.docs
              .map((pizza) => `- ${pizza.data().name}`)
              .sort()
              .join("\n")}`;
          } else {
    ...
    function selectPizza(agent) {
      ...
      const pizza =
        params.Pizza || agent.context.inputContexts.haspizza.parameters.name;
      agent.add(`行こう! ${count} x ${pizza}`);
    }
