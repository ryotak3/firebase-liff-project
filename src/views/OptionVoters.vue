<template>
  <section class="section">
    <div class="container">
      <h1 class="title" v-if="option">
        {{ voters.length }} 投票した人 "{{ option.text }}"
      </h1>
      <table>
        <tbody>
...
export default {
  ...
  mounted() {
    const main = this;
    firebase.pollsCollection.doc(main.pollId).onSnapshot(
      (docSnapshot) => {
        console.log(
        ...
        const index = poll.options.findIndex(
          (option) => option.value === main.optionValue
        );
        if (index >= 0) {
          let option = poll.options[index];
          main.option = option;
          main.voters = poll.voters.filter(
            (voter) => voter.option === main.optionValue
          );
        }
      },
      (err) => {
